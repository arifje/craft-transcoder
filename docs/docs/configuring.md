...
For video file replacements, Transcoder subscribes to Craft's dedicated `Assets::EVENT_AFTER_REPLACE_ASSET` event. It records a shared per-Asset source generation and queues the normal asynchronous inspection workflow. Videos, configured posters, lock files, and status keys use that immutable generation, so existing output is never mistaken for the replacement and superseded workers cannot publish over the current result. The queue worker reads a Craft-managed local copy of the exact replacement source, including for private remote volumes. The replacement request may enqueue this work even when `encodingServerNames` reserves actual FFmpeg work for a dedicated queue/encoding host. Calls that still pass an Asset URL/path string are matched back to the same versioned Asset when unambiguous.

The source-generation table is installed by the plugin migration. Run `php craft migrate/all` after updating. Older generation files are intentionally not deleted in the replacement request; clear Transcoder caches according to your retention policy to reclaim them. Ordinary Asset metadata and Entry saves remain inert.
